...
2. 填写注册信息：
   ...
   - 无需填写 Cloudflare Account ID（系统会自动获取；Token 需包含 区域（Zone）读取 权限）

### 获取 Cloudflare API Token

1. 登录 [Cloudflare 控制台](https://dash.cloudflare.com/)
2. 进入「API 令牌」页面：管理账户 → 账户 API 令牌（或：右上角头像 → 我的个人资料 → API 令牌）
3. 点击「创建令牌」
4. 选择「编辑区域 DNS」模板或创建自定义令牌
5. 权限（Permissions）建议：
   - 区域.DNS：编辑
   - 如需使用自定义主机名：区域.SSL 和证书：编辑
   - 如需使用 Tunnels：账户.Cloudflare Tunnel：编辑（仅查看可用 读取）
   - 建议额外添加：区域（Zone）：读取（用于读取域名列表/校验凭证）
6. 资源（Resources）选择：包含 → 所有区域（或选择需要管理的区域）
7. 复制生成的 Token 并保存
